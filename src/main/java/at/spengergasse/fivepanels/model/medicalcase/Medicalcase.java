package at.spengergasse.fivepanels.model.medicalcase;
...
public class Medicalcase extends BaseEntity {
    ...
    private void setOwner(Doctor owner) {
...
//        Map<String, Double> votePercentagePerVote =  voteCounts.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> entry.getValue() / members.size()));
    ...
    public void castVote(Doctor doctor, String answer, int percentage) {
        isNotNull(doctor, "user");
        isNotBlank(answer, "answer");
        isGreaterThanOrEqual(percentage, "percentage", 0, "zero");

        if (!(votes.containsKey(doctor.getId())))
            votes.put(doctor.getId(), new HashSet<>());

        if (votes.get(doctor.getId()).stream().mapToInt(Vote::getPercentage).sum() + percentage > 100)
