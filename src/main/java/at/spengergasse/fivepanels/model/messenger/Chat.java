...
public class Chat extends BaseEntity {
    ...
    public Chat addMember(Doctor doctor, Doctor doctorToAdd) {
        isNotNull(doctorToAdd, "userToAdd");
        isNotNull(doctor, "userToAdd");
        ...
        if (!groupChat) {
            ...
            sb.append(doctorToAdd.getProfile().getName());
            Set<UUID> newMembers = new HashSet<>(members);
            newMembers.remove(doctor.getId());
            newMembers.add(doctorToAdd.getId());
            Chat chat = doctor.createGroupChat(sb.toString(), newMembers);
            doctorToAdd.getChats().add(chat);
            members.forEach(uuid -> UserRepository.findById(uuid).ifPresent(user1 -> user1.getChats().add(chat)));
            return chat;
        }

        if (members.contains(doctorToAdd.getId()))
            throw new MessengerException(STR."addMember(): user is already a member of this chat");

        if (!(doctor.getSocials().getFriends().contains(doctorToAdd.getId())))
            throw new MessengerException("addMember(): user is not friends with userToAdd");

        members.add(doctorToAdd.getId());
        doctorToAdd.getChats().add(this);
        return this;
    }

    public void removeMember(Doctor doctor) {
        if (!groupChat)
            throw new MessengerException(STR."removeMember(): can not remove a member from a direct chat");
        isNotNull(doctor, "user");
        hasMaxSize(members, 513, "members");

        if (!(members.contains(doctor.getId())))
        ...
        members.remove(doctor.getId());
        doctor.getChats().remove(this);
    }
    ...
    public void sendMessage(Doctor doctor, TextContent content, List<Media> attachments) {

        if (!(members.contains(doctor.getId())))
            throw new MessengerException("sendMessage(): User is not a member of this chat");
        addToHistory(new Message(doctor, Instant.now(), content, attachments, Status.SENT));
    }
     ...
     *
     * @param doctor the user that wants to view the chat
    ...
    public String displayName(Doctor doctor) {
        if (groupChat) {
            return name;
        }
        AtomicReference<String> displayName = new AtomicReference<>();
        members.stream().filter(uuid ->
                doctor.getId() != uuid).findFirst().flatMap(UserRepository::findById).ifPresent(user1 ->
